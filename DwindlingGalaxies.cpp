// Definitions for the varibales and functions declared in the DwindlingGalaxies.h file.
    ...
    void DwindlingGalaxies::displayHighScore(){
        ...
        // Draw Values on Screen     
            ...
            // Track While Screen Scrolling
                if(player->get_x() > WindowSize.x*1.5){
                    // Display Names
                        showHighScore(((1.5*WindowSize.x)-210), (WindowSize.y/2+265)-(50*i), userScore[i].second, win, &highScoreText);
                    // Display Scores
                        showHighScore(((1.5*WindowSize.x)+130), (WindowSize.y/2+265)-(50*i), to_string(userScore[i].first), win, &highScoreText);
                }
                else if(player->get_x() < WindowSize.x*-0.5){
                    // Display Names
                        showHighScore(((-0.5*WindowSize.x)-210), (WindowSize.y/2+265)-(50*i), userScore[i].second, win, &highScoreText);
                    // Display Scores
                        showHighScore(((-0.5*WindowSize.x)+130), (WindowSize.y/2+265)-(50*i), to_string(userScore[i].first), win, &highScoreText);
                }
                else{
                    // Display Names
                        showHighScore((player->get_x()-210), (WindowSize.y/2+265)-(50*i), userScore[i].second, win, &highScoreText);
                    // Display Scores
                        showHighScore((player->get_x()+130), (WindowSize.y/2+265)-(50*i), to_string(userScore[i].first), win, &highScoreText);
                }
            }
    }
...
// PLAYER CONTROLS
    ...
    void DwindlingGalaxies::keyboardControls(Event event){
        if(menu.get_title() == false || menu.get_menu() == false || menu.get_easy() == false || menu.get_medium() == false || menu.get_expert() == false || menu.get_error() == false || menu.get_highScore() == false || 
            menu.get_options() == false || menu.get_controls() == false || menu.get_about() == false || menu.get_quit() == false || menu.get_pause() == false || menu.get_confirm() == false || 
            menu.get_gameOver() == false && num_lives != 0 || menu.get_gameOverMenu() == false){
             // Player Controls
